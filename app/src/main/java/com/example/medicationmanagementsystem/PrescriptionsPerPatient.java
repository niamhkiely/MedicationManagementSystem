...
public class PrescriptionsPerPatient extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState){
        ...
        Cursor data = myDB.getPrescriptionsPerPatient(patientid);
        int numRows = data.getCount();
        //if there is no rows in the database
        if(numRows == 0){
            Toast.makeText(PrescriptionsPerPatient.this, "There are no medications being prescribed to patient", Toast.LENGTH_LONG).show();
        }
        else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String newText) {
                ...
                ArrayAdapter<String> adapter = new ArrayAdapter<String>(PrescriptionsPerPatient.this,R.layout.prescription_view,listViews);
                listView = (ListView) findViewById(R.id.listview);
                listView.setAdapter(adapter);
                return true;
            }
